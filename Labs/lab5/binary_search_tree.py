    ...
    def delete(self, key): # deletes node containing key
        ...
        if self.search(key):
            if self.root.left == None and self.root.right == None:
                ...
                if self.root.left:
                    #self.root.key = self.root.left.find_max()[0]
                    #self.root.key = self.root.left.find_max_node(self.root.left)
                    self.root.key = self.find_max_node(self.root.left)
                    #self.root.key = self.find_max()[0]
                    self.delete_helper(self.root.key, self.root)
                else:
                    #self.root.key = self.root.right.find_min()
                    #self.root.key = self.root.right.find_min_node(self.root.right)
            ...
            if current_Node.left: # unneeded
                if current_Node.left.key == key:
                    ...
                    if current_Node.left.left == None and current_Node.left.right == None: # leaf case
                        ...
                        #current_Node.left.key = current_Node.left.left.find_max()[0] # replace with max of left subtree
                        #current_Node.left.key = current_Node.left.left.find_max_node(self.root.left.left)
            ...
            if current_Node.right:
                if current_Node.right.key == key:
                    ...
                    if current_Node.right.left == None and current_Node.right.right == None: # leaf case
                        ...
                        #current_Node.right.key = current_Node.right.left.find_max()[0] # replace with max of left subtree
                        #current_Node.right.key = current_Node.right.left.find_max_node(self.root.right.left)
...
'''
###JACOB you have a screen shot using a queue##
